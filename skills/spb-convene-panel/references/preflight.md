# Stage 0 — Preflight (RP)

Probe the SynthPanel MCP toolset. Cheap fail-fast before any user-facing work.

## What to check

1. **`run_panel` is reachable.** Required. If absent, stop with the error below.
2. **`extend_panel` is reachable.** Warn-only — workflow continues. Note the
   gap in the Stage 7 readback if the user might want to extend later.
3. **Schema version.** If the server advertises a `schema_version` outside the
...
`run_panel` reachable. Note any warnings on a scratch buffer for the readback.
